...
class TestDataLoad:
    ...
    def test_data_load_empty_folder_download(self, mock_dataloader, mock_transform_subset, mock_split, 
                                           mock_cifar, mock_mkdir, mock_exists, mock_listdir, mock_subprocess):
        ...
        mock_transform_subset.side_effect = [mock_transformed_subset1, mock_transformed_subset2]
        ...
        mock_subprocess.assert_called_once_with([
            "dvc", "import-url", config['dataset']['s3_path'], config['dataset']['data']
        ], check=True)
    ...
    @patch('utils.dataloader.TransformSubset')
    @patch('utils.dataloader.DataLoader')
    def test_data_load_existing_data_update(self, mock_dataloader, mock_transform_subset, mock_split, 
                                          mock_cifar, mock_exists, mock_listdir, mock_subprocess):
        ...
        # Verify update was called instead of download
        mock_subprocess.assert_called_once_with([
            "dvc", "update", config['dataset']['dvc_path']
        ], check=True)
    ...
    @patch('utils.dataloader.TransformSubset')
    @patch('utils.dataloader.DataLoader')
    def test_data_load_folder_not_exists(self, mock_dataloader, mock_transform_subset, mock_split, 
                                       mock_cifar, mock_mkdir, mock_exists, mock_listdir, mock_subprocess):
        ...
        mock_exists.side_effect = [False, True]  # First call False, second True after mkdir
        mock_listdir.return_value = []
        ...
        mock_mkdir.assert_called_once_with(config['dataset']['data'])
    ...
    @patch('os.listdir')
    @patch('os.path.exists')
    def test_data_load_subprocess_error(self, mock_exists, mock_listdir, mock_subprocess):
        """Test data_load when subprocess fails"""
        config = {
            'dataset': {
                'data': './data',
                's3_path': 's3://bucket/data',
    ...
    @patch('utils.dataloader.TransformSubset')
    @patch('utils.dataloader.DataLoader')
    def test_data_load_different_split_ratio(self, mock_dataloader, mock_transform_subset, mock_split, 
                                           mock_cifar, mock_exists, mock_listdir, mock_subprocess):
