...
@mock.patch("wandb.login")
@mock.patch("wandb.init")
def test_begin_wandb_no_wandb_dir(mock_init, mock_login, mock_isdir, mock_listdir, mock_exists):
...
@mock.patch("wandb.login")
@mock.patch("wandb.init")
def test_begin_wandb_with_existing_runs(mock_init, mock_login, mock_isdir, mock_listdir, mock_exists):
...
def test_model_export_detailed(mock_randn, mock_onnx_export, mock_json_dump, mock_file):
    ...
    with mock.patch("utils.utils.datetime") as mock_datetime:
        mock_datetime.now.return_value.strftime.return_value = "20240101_120000"
        
        with mock.patch('builtins.print') as mock_print:
            model_export(model, device, config)
        
        # Check that config was written to file
        mock_json_dump.assert_called_once_with(config, mock_file.return_value.__enter__.return_value, indent=4)
        
        # Check that dummy input was created correctly
        mock_randn.assert_called_with(1, 3, 32, 32)
        
        # Check ONNX export parameters
        mock_onnx_export.assert_called_once_with(
            model,
            dummy_tensor.to(device),
            "Model_20240101_120000.onnx",
            opset_version=11,
            input_names=["input"],
            output_names=["output"],
            dynamic_axes={
        ...
        # Check print message
        mock_print.assert_called_with("Model & config file saved as: Model_20240101_120000.onnx & Config_Model_20240101_120000.onnx.txt")
