...
def data_load(config):
   ...
   # ])
    ...
    #Downloading dataset
    trainset = torchvision.datasets.CIFAR10(root=config['dataset']['data'], train=True, download=True, transform=transform)
    ...
    train_loader = torch.utils.data.DataLoader(train_subset, batch_size=config['train']['batch_size'], 
                                               shuffle=True, num_workers=config['train']['num_workers'])

    val_loader = torch.utils.data.DataLoader(val_subset, batch_size=config['train']['batch_size'], 
